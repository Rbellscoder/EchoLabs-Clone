...
#   -d '{"prompt": "Hello from Chatterbox [chuckle].", "voice_key": "voices/system/<voice-id>", "save_to_r2": true}' \
#   --output output.wav
...
with image.imports():
    ...
    def get_r2_client():
        """R2 client for WRITES (the CloudBucketMount stays read-only)."""
        return boto3.client(
            "s3",
    ...
    class TTSRequest(BaseModel):
        ...
        save_to_r2: bool = Field(
            default=False,
            description="If true, upload the generated audio to R2 under outputs/ "
            "and return JSON {key, bucket} instead of streaming the wav bytes.",
        )
...
class Chatterbox:
    ...
    def serve(self):
        ...
        def generate_speech(request: TTSRequest):
            ...
            if request.save_to_r2:
                key = f"outputs/{uuid.uuid4()}.wav"
                try:
                    r2 = get_r2_client()
                    r2.put_object(
                    ...
                    raise HTTPException(
                        status_code=500,
                        detail=f"Generated audio but failed to upload to R2: {e}",
                    )
                return {"key": key, "bucket": R2_BUCKET_NAME}
        ...
        @web_app.get("/audio/{key:path}", responses={200: {"content": {"audio/wav": {}}}})
        def get_audio(key: str):
            """Fetch a previously generated clip saved under outputs/ by its R2 key."""
            try:
                r2 = get_r2_client()
                obj = r2.get_object(Bucket=R2_BUCKET_NAME, Key=key)
                data = obj["Body"].read()
            except Exception as e:
                raise HTTPException(status_code=404, detail=f"Audio not found for key '{key}': {e}")
